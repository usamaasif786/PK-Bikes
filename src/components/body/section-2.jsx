...
const Rating = ({ rating, starSize }) => (
  <div>
    {[...Array(MAX_RATING)].map((_, index) => (
      <RatingStar key={index} filled={index < rating} size={starSize} />
    ))}
  </div>
);
...
const Tires = ({ all_categories }) => (
  <div className="row d-flex justify-content-start">
    {all_categories.tireData.map((tire, index) => (
      <div
        key={`tire_${index}`}
        className="col-12 col-md-3 m-4 p-3 bd-highlight bg-white b-radius"
      >
          ...
          <div className="row justify">
            <div className="col-2 m-4 bold justify">
              ...
              <button
                type="button"
                className="btn btn-outline-dark m-1 fw-bold"
                onClick={() => handleTabClick("motoroil")}
              >
                Moter Oils
              </button>{" "}
              <br />
              <button
                type="button"
                className="btn btn-outline-dark m-1 fw-bold"
                onClick={() => handleTabClick("breaksystem")}
              >
                Break System
              </button>{" "}
              <br />
              ...
              <button
                className="p-2 mt-4 d-flex align-items-center bold"
                style={{
                  height: "auto",
            ...
            <div className="col">
              {activeTab === "tires" && (
                <Tires all_categories={all_categories} />
              )}
              {activeTab === "accessories" && (
                <Accessories all_categories={all_categories} />
              )}
              {activeTab === "filters" && (
                <Filters all_categories={all_categories} />
              )}
              {activeTab === "motoroil" && (
                <MotorOil all_categories={all_categories} />
              )}
              {activeTab === "breaksystem" && (
                <BreakSystem all_categories={all_categories} />
              )}
              {activeTab === "engine" && (
                <Engine all_categories={all_categories} />
              )}
